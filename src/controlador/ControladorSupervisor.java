...
public class ControladorSupervisor implements ActionListener{
    ...
    public ControladorSupervisor(VistaSupervisor visS) {
       ...
       this.vistaSupervisor.btnPdfF.addActionListener(this);
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(e.getSource()==vistaSupervisor.btnPdfF){
            generarPdfTareasFinalizadas();
        }
    }
...
//         encabezado.setHorizontalAlignment(All);
    ...
    public void generarPdfTareasFinalizadas() {
         Document documento = new Document();
         
           
        try {
         String lugarGuardado=System.getProperty("user.home");
         File file = new File(lugarGuardado + "/Desktop/TAREAS_FINALIZADAS.pdf");
        if (!file.getParentFile().exists()) {
        file.getParentFile().mkdirs(); // Crea el directorio si no existe
        }
         PdfWriter.getInstance(documento,new FileOutputStream(lugarGuardado+"/Desktop/TAREAS_FINALIZADAS.pdf"));
         documento.open();
         Image img1= Image.getInstance("src/img/grupo.png");
         Date fecha=new Date();
         SimpleDateFormat formatoFecha= new SimpleDateFormat("dd/MM/yyyy");
...
//         encabezado.setHorizontalAlignment(All);
         ...
         Paragraph subtitulo = new Paragraph();
         subtitulo.add("Lista Ropa entregada\n\n");
         
         documento.add(encabezado);
         documento.add(subtitulo);
         documento.add(this.crpc.daClinica.listarpdfTareasFinalizadas());
        documento.close();
        JOptionPane.showMessageDialog(vistaSupervisor, "Pdf creado");
        } catch ( Exception e) {
            JOptionPane.showMessageDialog(null,"Exception en el controlador"+e.toString());
