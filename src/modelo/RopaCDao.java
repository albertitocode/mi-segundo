...
public class RopaCDao implements Crud<RopaClinica>{
    ...
    public List listar(){
        ...
        String sql="select rp.rp_id,rp.rpc_id,rp.rp_nombre,rp.rp_marca,rp.rp_descripcion,rp.rp_color,c.categoria_nombre,es.estado_nombre,rp.rpc_descripcion_dano,us.persona_nombre,rp.rpc_cliente, rp.rpc_precio,rp.rpc_tiempoDeEntrega FROM ropa_clinica rp JOIN categoria c ON rp.rp_categoria=c.categoria_id JOIN estado_ropa es ON rp.rpc_estado=es.estado_id JOIN usuarios us ON rp.rpc_usuario=us.usuario_id";
        try{
            ...
            while(rs.next()){
                ...
                rpc.setRpc_precio(rs.getInt(12));
                rpc.setRpc_tiempoDeEntrega(rs.getInt(13));
    ...
    public PdfPTable listarpdf() throws DocumentException{
        ...
        Font letraBlanca = new Font(Font.FontFamily.HELVETICA, 12, Font.BOLD, BaseColor.WHITE);
         PdfPTable mitablapdf= new PdfPTable(13);   
...
//         mitablapdf.setWidths(TamanoTabla);
         ...
         PdfPCell cel12=new PdfPCell(new Phrase("Precio",letraBlanca));
         cel12.setBackgroundColor(verdi);
         PdfPCell cel13=new PdfPCell(new Phrase("Tiempo de entrega\n por dias",letraBlanca));
         cel13.setBackgroundColor(verdi);
         ...
         mitablapdf.addCell(cel12);
         mitablapdf.addCell(cel13);
        String sql="select rp.rp_id,rp.rpc_id,rp.rp_nombre,rp.rp_marca,rp.rp_descripcion,rp.rp_color,c.categoria_nombre,es.estado_nombre,rp.rpc_descripcion_dano,us.persona_nombre,rp.rpc_cliente,rp.rpc_precio,rp.rpc_tiempoDeEntrega FROM ropa_clinica rp JOIN categoria c ON rp.rp_categoria=c.categoria_id JOIN estado_ropa es ON rp.rpc_estado=es.estado_id JOIN usuarios us ON rp.rpc_usuario=us.usuario_id";
        try{
            conex=(Connection) conectar.getConnection();
            ps=(PreparedStatement) conex.prepareStatement(sql);
...
//                PdfPTable tablapdf= new PdfPTable(11);
                do{
                    ...
                    mitablapdf.addCell(rs.getString(12));
                    mitablapdf.addCell(rs.getString(13));
                }while(rs.next());
//                documento.add(tablapdf);
    ...
    public PdfPTable listarpdfTareasFinalizadas() throws DocumentException{
        BaseColor verdi= new BaseColor(0,102,0);
        Font letraBlanca = new Font(Font.FontFamily.HELVETICA, 12, Font.BOLD, BaseColor.WHITE);
         PdfPTable mitablapdf= new PdfPTable(13);   
//         float[] TamanoTabla=new float[]{10f,10f,10f};
...
//       int r;
        ...
        try{
            ...
            ps.setInt(12, rpclinica.getRpc_precio());
...
//       int r;
        ...
        try{
            ...
            ps.setInt(12, rp.getRpc_precio());
            ps.setInt(13, rp.getRpc_tiempoDeEntrega());
            ps.executeUpdate();
            return 1;
...
//        }
     ...
     public int setActualizarr(RopaClinica rp,int cat,int esta,int usus){
        String sql="UPDATE ropa_clinica SET rp_nombre=?,rp_marca=?,rp_descripcion=?,rp_color=?,rp_categoria=?,rpc_estado=?,rpc_descripcion_dano=?,rpc_usuario=?,rpc_cliente=?,rpc_precio=?,rpc_tiempoDeEntrega=? WHERE rpc_id="+rp.getRpc_id();
        ...
        try{
            ...
            ps.setInt(8, usus);
            ps.setInt(9, rp.getRpc_cliente());
            ps.setInt(10, rp.getRpc_precio());
            ps.setInt(11, rp.getRpc_tiempoDeEntrega());
            ps.executeUpdate();
