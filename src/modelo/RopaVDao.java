...
//       int r;
    ...
    public int setEliminar(int rpv_id) {
      ...
      public List getRopav(){
        List<RopaVenta> datos=new ArrayList<RopaVenta>();
        String sql="SELECT rpv.rp_id,rpv.rpv_id,rpv.rp_nombre,rpv.rp_marca,rpv.rp_descripcion,rpv.rp_color,c.categoria_nombre,rpv.rpv_precio,rpv.rpv_stock AS categoria FROM ropa_venta rpv JOIN categoria c ON rpv.rp_categoria = c.categoria_id WHERE rpv.rpv_stock<=5";
        try{
            conex=(Connection) conectar.getConnection();
            ps=(PreparedStatement) conex.prepareStatement(sql);
            rs=ps.executeQuery();
       ...
       public int setActualizarStock(){
        String sql="UPDATE ropa_venta SET rpv_stock=(rpv_stock+10) WHERE rpv_stock<=5";
